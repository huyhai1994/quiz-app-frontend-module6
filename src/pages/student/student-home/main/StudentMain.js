...
import defaultCardQuiz from '../../../../asset/quiz show-rafiki.svg';
...
const StudentMain = () => {
    ...
    const formik = useFormik({
        initialValues: {
            ...
            const filtered = quizzes.filter(quiz => (quiz.title && quiz.title.toLowerCase().includes(searchQuery)) || (quiz.category && quiz.category.name && quiz.category.name.toLowerCase().includes(searchQuery)));
    ...
    return (<Box sx={{padding: '16px'}}>
        ...
        {/* Thanh tìm kiếm */}
        <div
            style={{width: '100%', backgroundColor: 'var(--color-secondary)', padding: '2px', borderRadius: '8px'}}>
        ...
        <div className="p-3">
            {Object.keys(groupedQuizzes).map(categoryName => (<div key={categoryName}>
                <h1>{categoryName}</h1>
                <Grid container spacing={3} sx={{marginTop: 1, height: '100%'}} justifyContent="center"
                    ...
                    {groupedQuizzes[categoryName]
                        ...
                        .map((quiz) => (<Grid item xs={12} sm={6} md={4} key={quiz.id}>
                            <Card sx={{
                                maxWidth: 345,
                                boxShadow: 3,
                                ...
                                <CardContent>
                                    <Typography gutterBottom variant="h5"
                                                component="div">{quiz.title}</Typography>
                                    <Typography variant="body2"
                                                color="text.secondary">{quiz.category?.description || "No description available"}</Typography>
                                    <Box display="flex" justifyContent="space-between">
                                        <Typography variant="body2"
                                                    color="text.secondary">Có {quiz.questions?.length || 0} câu
                                            hỏi</Typography>
                                        <Typography variant="body2"
                                                    color="text.secondary">{quiz.category?.playCount || 0} lượt
                                            chơi</Typography>
                                    </Box>
                                </CardContent>
                            </Card>
